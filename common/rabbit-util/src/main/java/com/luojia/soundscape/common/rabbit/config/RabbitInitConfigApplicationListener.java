...
import com.luojia.soundscape.common.rabbit.entity.SoundscapeCorrelationData;
...
public class RabbitInitConfigApplicationListener implements ApplicationListener<ApplicationReadyEvent> {
    ...
    private void setupCallbacks() {
        ...
        this.rabbitTemplate.setReturnsCallback(returned -> {
            ...
            SoundscapeCorrelationData soundscapeCorrelationData = JSON.parseObject(correlationDataStr, SoundscapeCorrelationData.class);
            //todo 方式一:如果不考虑延迟消息重发 直接返回
            if(soundscapeCorrelationData.isDelay()){
                return;
            }
            //2.调用消息重发方法
            this.retrySendMsg(soundscapeCorrelationData);
        });
    }
...
//        //获取相关数据
//        SoundscapeCorrelationData gmallCorrelationData = (SoundscapeCorrelationData) correlationData;
//
//        //获取redis中存放重试次数
//        //先重发，在写会到redis中次数
