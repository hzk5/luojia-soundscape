					...
					<view class="gui-flex gui-m-t-20">
						<view class="gui-flex1">
							<view class="gui-flex">
								<image
									mode="aspectFill"
									:src="resolveAnnouncerAvatarUrl(albumDetailInfo.announcer?.avatarUrl)"
									@error="handleAnnouncerAvatarError"
						...
						<view class="gui-bg-black-opacity1 gui-text-center" style="width: 150rpx; height: 60rpx; line-height: 60rpx; border-radius: 20rpx" @click="handleFollowAnnouncer">
							<text class="gui-icons gui-color-white gui-text-small">&#xe6c7; {{ isFollowingAnnouncer ? '已关注' : '关注' }}</text>
						</view>
					</view>
...
const announcerFollowStorageKey = `local-announcer-follow:${props.id}`
const savedAnnouncerFollowState = uni.getStorageSync(announcerFollowStorageKey)
const isFollowingAnnouncer = ref(
	savedAnnouncerFollowState === true || savedAnnouncerFollowState === "true",
)

const resolveAnnouncerAvatarUrl = (avatarUrl?: string) => {
	if (!avatarUrl || avatarUrl.includes("thirdwx.qlogo.cn")) {
		return DEFAULT_AVATAR_URL
	}
	return avatarUrl
}

const handleAnnouncerAvatarError = () => {
	if (albumDetailInfo.value.announcer) {
		albumDetailInfo.value.announcer.avatarUrl = DEFAULT_AVATAR_URL
	}
}

// 当前后端尚无主播关注接口，先使用本地状态保证交互完整。
const handleFollowAnnouncer = () => {
	isFollowingAnnouncer.value = !isFollowingAnnouncer.value
	uni.setStorageSync(announcerFollowStorageKey, isFollowingAnnouncer.value)
}
...
const getIsSubscribe = async () => {
	try {
		...
		isSubscribe.value = Boolean(res.data)
	} catch (error) {
		console.log(error)
...
onShow(() => {
  // 处理支付成功后的回显问题
  if (zPagingRef.value && typeof zPagingRef.value.refresh === "function") {
