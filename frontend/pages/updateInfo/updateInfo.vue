...
const avatarUrlList = reactive<string[]>(initialAvatarUrl ? [initialAvatarUrl] : [])

const syncAvatarUrlList = (value: unknown) => {
  const list = Array.isArray(value) ? value.filter(Boolean) : []
  avatarUrlList.splice(0, avatarUrlList.length, ...list)
}
...
const uploadImgSuccess = (res: any) => {
  ...
  avatarUrlList.splice(0, avatarUrlList.length, res.data)
