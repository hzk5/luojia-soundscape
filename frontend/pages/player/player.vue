...
import { LEGACY_MEDIA_HOST, MEDIA_BASE_URL } from "../../config/confjg"
 ...
 * @returns {*}
 */
const resolvePlayableMediaUrl = (mediaUrl: string | undefined, trackId: number) => {
	if (!mediaUrl) return ""
	try {
		if (LEGACY_MEDIA_HOST && new URL(mediaUrl).host === LEGACY_MEDIA_HOST) {
			const demoFile = Number(trackId) % 2 === 0 ? "xiaomaolu.mp3" : "jiafeimao.mp3"
			return `${MEDIA_BASE_URL}/${demoFile}`
		}
	} catch (error) {
		console.warn("无法解析媒体地址", error)
	}
	return mediaUrl
}
...
const createBgAudioManager = () => {
	if (bgAudioManager) {
		...
		// 设置了src以后会自动播放
		bgAudioManager.src = resolvePlayableMediaUrl(
			trackInfo.value?.mediaUrl,
			trackInfo.value?.id || audios.trackId,
		);
...
const getIsCollect = async () => {
	try {
		...
		isCollect.value = Boolean(res.data)
	} catch (error) {
		console.log(error)
